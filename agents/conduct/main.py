...
def process_conduct(state_dict):
    state = A2AState(**state_dict)
    state.data['conduct_processed'] = True
    state.next_agent = 'closeout'
    collection = chroma_client.get_or_create_collection('audit_logs')
    collection.add(documents=[f"Conduct processed for {state.user_query}"], ids=[state.trace_id])
    return state.dict()

@app.post('/process')
async def process_state(state: A2AState):
    with langfuse_wrapper.create_trace(name='conduct_agent', session_id=state.trace_id) as trace:
        observation = langfuse_wrapper.create_observation(trace, name='process_conduct', input=state.dict())
        try:
            result = process_conduct(state.dict())
