...
def process_etl_orchestrator(state_dict):
    state = A2AState(**state_dict)
    state.data['etl_orchestrator_processed'] = True
    state.next_agent = 'data_profiler'
    collection = chroma_client.get_or_create_collection('audit_logs')
    collection.add(documents=[f"Etl_Orchestrator processed for {state.user_query}"], ids=[state.trace_id])
    return state.dict()

@app.post('/process')
async def process_state(state: A2AState):
    with langfuse_wrapper.create_trace(name='etl_orchestrator_agent', session_id=state.trace_id) as trace:
        observation = langfuse_wrapper.create_observation(trace, name='process_etl_orchestrator', input=state.dict())
        try:
            result = process_etl_orchestrator(state.dict())
