...
def process_analyst(state_dict):
    state = A2AState(**state_dict)
    state.data['analyst_processed'] = True
    state.next_agent = 'insights'
    collection = chroma_client.get_or_create_collection('audit_logs')
    collection.add(documents=[f"Analyst processed for {state.user_query}"], ids=[state.trace_id])
    return state.dict()

@app.post('/process')
async def process_state(state: A2AState):
    with langfuse_wrapper.create_trace(name='analyst_agent', session_id=state.trace_id) as trace:
        observation = langfuse_wrapper.create_observation(trace, name='process_analyst', input=state.dict())
        try:
            result = process_analyst(state.dict())
